...
function getAllChangedPackages(options, packageInfos) {
    ...
    const changes = [...((0, workspace_tools_1.getChanges)(branch, cwd) || []), ...((0, workspace_tools_1.getStagedChanges)(cwd) || [])];
    ...
    const nonIgnoredChanges = changes.filter(moddedFile => {
        const ignorePattern = ignorePatterns.find(pattern => (0, minimatch_1.default)(moddedFile, pattern, { matchBase: true }));
...
function getChangedPackages(options, packageInfos) {
    ...
    const changedFilesResult = (0, workspace_tools_1.git)(['diff', '--name-only', '--relative', '--no-renames', '--diff-filter=A', `${branch}...`], { cwd });
    if (!fs_extra_1.default.existsSync(changePath) || !changedFilesResult.success) {
        return changedPackages;
    }
    const changeFiles = changedFilesResult.stdout.split('\n').filter(name => path_1.default.dirname(name) === changeDir);
